...
FPRegister[0][0] = 56.45
FPRegister[8][0] = 864.56
FPRegister[0][0] = 100
FPRegister[8][0] = 25.2555
...
#RAM
Main_Memory = [0 for i in range(2048)]
Main_Memory[500] = 10000
...
def LogicUnit(sno,start_time):	#Logic operations
	...
	if(ReservationStLogicShift[sno][3] == 'XOR'):
		...
		op3 = ~(op1 & op2)
	elif(ReservationStLogicShift[sno][3] == 'AND'):
		op3 = op1 &op2
	else:
		op3 = op1 | op2
	...
	for i in range(1,4):
		if LoadBuffer[i][0] == clock:
			if cdb == False:
				...
				sr_in = int(LoadBuffer[i][3].split('R')[1])
				LoadBuffer[i][3] = 0

				RS_Name = RegisterStatus[int(LoadBuffer[i][4].split('R')[1])][1]
				dr_in = int(LoadBuffer[i][4].split('R')[1])
				
				FPRegister[dr_in][0] = Main_Memory[FPRegister[sr_in][0]]
				# src_in = int(LoadBuffer[i][3].split('R')[1])
				# d_rin = 0
				# for j in range(16):
				# 	if(RegisterStatus[j][1] == RS_Name):
				# 		d_rin = j
				# FPRegister[d_rin] = Main_Memory[d_rin]
		...
		if StoreBuffer[i][0] == clock:
			if cdb == False:
				...
				sr_in = int(StoreBuffer[i][3].split('R')[1])
				StoreBuffer[i][3] = 0

				RS_Name = RegisterStatus[int(StoreBuffer[i][4].split('R')[1])][1]
				RegisterStatus[int(StoreBuffer[i][4].split('R')[1])][1] = 0
				dr_in = int(StoreBuffer[i][4].split('R')[1])
				StoreBuffer[i][4] = 0
				Main_Memory[FPRegister[dr_in][0]] = FPRegister[sr_in][0]
				StoreBuffer[i][5] = 0
				StoreBuffer[i][6] = 0
				StoreBuffer[i][7] = 0
				StoreBuffer[i][8] = 0
...
def MainFunction():
	...
	while(1):			
		...
		print("Register File DATA")

		for i in range(0,16):
			print('R'+str(i),end =' =')
			if(RegisterStatus[i][1] == 0):
				print(FPRegister[i][0])
			else:
				print("BUSY")
			# if(FPRegister[i][1])
		print("valat 150",Main_Memory[150])
